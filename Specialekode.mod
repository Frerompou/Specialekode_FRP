// Referencer:
// Modellen er originalt replikeret af:  Sören Karau (Soerenkarau@web.de 2
...
//**************************************************************************
// Egne justeringer ift. originalkode
// 1. Husholdningernes ejerskab er fjernet fra modellen. Dette er gjort ved at 
// fjerne deres beholdninger i steady-state, samt udelade muligheden for at koebe stats- og virksomhedsobligationer.
// 2. Jeg har modsat originalmodellen indfoert mulighed for centralbanken kan saenke den nominelle rente til nul, hvor den vil vaere bundet.
// 3. Chok til modellen er ikke en del af originalkoden, hvormed jeg selv har udfoert disse. For hver figur
// i opgaven er disse praesenteret i sektion 4 (nederst). For hvert chok kan den tilhoerende renteregel findes i model-ligning 32 i koden.   
//--------------------------------------------------------------------------
// 1. Variable i modellen
//--------------------------------------------------------------------------
...
//  parameters:
    ...
    Psi=0; //0.50000000;
    Sigma=0; //0.75000000;
    kappa_s=0; //1.00000000;
    kappa_b=0; //1.00000000;
...
//  Steady-State vaerdier:
	...
	Kh_bar	=	0 ;//exp(K_ss)*Psi-kappa_s*betta*(exp(Rk_ss)-exp(R_ss));	
	Bh_bar	=	0 ;//B*Sigma-kappa_b*Delta*betta*(exp(Rk_ss)-exp(R_ss));	
	Kb_ss	=	log(exp(K_ss)); //-Kh_bar-kappa_s*betta*(exp(Rk_ss)-exp(R_ss)));	
	Bb_ss	=	log(B); //-Bh_bar-kappa_b*Delta*betta*(exp(Rk_ss)-exp(R_ss)));	
	Kh_ss	=	0; //log(exp(K_ss)-exp(Kb_ss));	
	Bh_ss	=	0; //log(B-exp(Bb_ss));	
...
model;
    ...
    ir = zlb*(1/exp(ir_ss)-1)+(1-zlb)*irstar; //Renteregel 1: Denne laaser på zlb, bruges i forward guidance
    //ir = zlb*(0)+(1-zlb)*irstar; // Renteregel 2: Denne laaser den nominelle rente paa 2pct i fire perioder, hvorefter Taylor-regel genoptages 
    //ir = max(irstar,1/exp(ir_ss)-1); // Renteregel 3: Denne foelger Taylor-reglen, men binder ved ZLB hvor den nominelle rente er nul.
    ...
    Kb*exp(Kb_ss) + psi*exp(K_ss) + K*exp(K_ss)*(psi_ss - 1) =0;// - R*betta*kappa_s*exp(R_ss) + Rk(+1)*betta*kappa_s*exp(Rk_ss) + betta*kappa_s*varrho*(exp(R_ss) - exp(Rk_ss)) - betta*kappa_s*varrho(+1)*(exp(R_ss) - exp(Rk_ss)) + Kh_bar*ksi*switch_Kh_bar*exp(ksi_ss)*exp(ksi_ss)^(switch_Kh_bar - 1)=0;

    //58. Financial Intermediaries' Long term Treasury holdings
    Bb*exp(Bb_ss) + B*Gamma =0 ; //+ Delta*betta*kappa_b*varrho*(exp(R_ss) - exp(Rk_ss)) - Delta*betta*kappa_b*varrho(+1)*(exp(R_ss) - exp(Rk_ss)) - Delta*R*betta*kappa_b*exp(R_ss) + Delta*Rk(+1)*betta*kappa_b*exp(Rk_ss)=0;

    //59. HHs direct corporate bond holdings
    Kh*exp(Kh_ss)=0; //Kb*exp(Kb_ss) - K*exp(K_ss) =0;

    //60. HHs direct long term Treasury holdings
    Bh*exp(Bh_ss)=0; //Bb*exp(Bb_ss)  + 
...
// Figur 5.1
//--------------------------------------------------------------------------- 
// Chok 1 (Opkoeb af statsobligationer med peak-effekt paa 30 pct. af Y, Fastholdelse af nominel rente paa 2 pct. annualiseret i fire perioder)(Renteregel 2)
//var e_zlb;
//periods 1;
//values 1;
//var e_Gamma;
//periods 1;
//values 0.7517;
// Chok 2 (Opkoeb af virksomhedsobligationer med peak-effekt paa 30 pct. af Y, Fastholdelse af nominel rente paa 2 pct. annualiseret i fire perioder)(Renteregel 2)
//var e_zlb;
//periods 1;
//values 1;
//var e_psi;
...
// Figur 5.2
//--------------------------------------------------------------------------- 
// Chok 3 (Kapitalkvalitets-Chok uden QE. 0,5 pct. afskrivning af kapitalapparat i 4 perioder. ZLB binder)(Renteregel 3)
//  var e_ksi;
//  periods 1:4;
//  values 0.005;
// Chok 4 (Kapitalkvalitets-Chok med QE. 0,5 pct. afskrivning af kapitalapparat i 4 perioder. Opkoeb af 30% af Y i peak-effekt, hvoraf 25pct er virksomhedsobligationer og 75pct statsobligationer. Taylor-regel med ZLB)(Renteregel 3)
//  var e_ksi;
//  periods 1:4;
//  values 0.005;
//  var e_psi;
...
// Figur 5.3
//--------------------------------------------------------------------------- 
// Chok 5 (Kapitalkvalitets-Chok med QE og Taylor-regel. Samme som Chok 4)(Renteregel 3)
//  var e_ksi;
//  periods 1:4;
//  values 0.005;
//  var e_psi;
...
//  periods 1;
//  values 0.563775;
// Chok 6 (Kapitalkvalitets-Chok med QE og Forward Guidance. Samme som Chok 4. Dog afviger centralbanken fra Taylor-reglen ved at holde den nominelle rente på ZLB i 4 perioder (Renteregel 1)
//  var e_ksi;
//  periods 1:4;
//  values 0.005;
//  var e_psi;
